...
**NEW**: Now includes thermal and droop simulation for predicting print defects! See [THERMAL_SIMULATION.md](THERMAL_SIMULATION.md) for details.
...
- Outputs
  ...
  - **NEW**: Physics simulation data (temperature, droop, fusion analysis)
...
## New Features: Physics Simulation

VolCo now includes optional physics-based simulation to predict:

- 🌡️ **Thermal behavior**: Temperature tracking, cooling rates, layer fusion
- ⬇️ **Droop/Sagging**: Gravity effects on unsupported filament
- 🌉 **Bridging analysis**: Automatic detection of problematic bridges
- 📊 **Quality scoring**: Printability assessment with recommendations
...
# Enable physics simulation in your config
sim_config = {
    # ... standard settings ...
    "enable_thermal_simulation": True,
    "enable_droop_simulation": True,
    "material_type": "PLA"  # PLA, ABS, or PETG
}

output = run_simulation(gcode_path='file.gcode', sim_config=sim_config, ...)
...
📖 **[Full documentation](THERMAL_SIMULATION.md)** | 💡 **[Examples](examples/thermal_simulation_example.py)**
...
| **`enable_thermal_simulation`** | **NEW**: Enable temperature tracking and fusion detection. | false |
| **`enable_droop_simulation`** | **NEW**: Enable gravity-based droop calculation for unsupported segments. | false |
| **`material_type`** | **NEW**: Material type for physics simulation: "PLA", "ABS", or "PETG". | "PLA" |
